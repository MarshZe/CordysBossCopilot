...
客户列表接口（`account/page`）返回的跟进相关字段：

| 返回字段 | 含义 | 数据来源 | 是否可用 |
|---------|------|---------|---------|
| `followTime` | 最新跟进日期 | `customer.follow_time` 表字段，创建跟进记录时自动更新 | ✅ 可用 |
| `follower` | 最新跟进人 | `customer.follower` 表字段，创建跟进记录时自动更新 | ✅ 可用 |
| `latestFollowUpTime` | ~~最近跟进时间~~ | **未被任何 SQL 填充，永远为 null** | ❌ 不可用 |

**关键规则：判断客户是否有跟进，必须看 `followTime`，不能看 `latestFollowUpTime`。**

如果 `followTime` 为 null，说明该客户从未被录入过跟进记录。如果有值，说明最近一次跟进发生在该时间点。

### 跟进记录数量统计

当前系统**没有**专门的跟进记录总数统计接口。获取跟进数据的可用方式：

1. **`analysis customer-overview`** → `statCards` 中的 `followed` 卡片：返回在时间范围内**有跟进的客户数**（不是跟进记录条数）
2. **`crm follow record account`** → 需要 `sourceId`，只能查单个客户的跟进记录列表
3. **客户列表的 `followTime` 字段** → 可判断客户是否有过跟进，以及最近跟进时间

**报告中"跟进记录 X 条"这个指标目前无法精确获取。** 应替换为"已跟进达人 X 人"（来自 `analysis customer-overview` 的 `followed`）或"有跟进记录的达人占比 X%"。
